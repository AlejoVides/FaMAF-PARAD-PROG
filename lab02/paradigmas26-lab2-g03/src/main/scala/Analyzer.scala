...
object Analyzer {
  ...
  def detectEntities(text: String, dictionary: List[NamedEntity]): List[NamedEntity] = {
    ...
    dictionary filter (entity => lowerText.matches(s".*\\b${entity.text.toLowerCase}\\b.*"))
  }
  ...
  def countByType(entities: List[NamedEntity]): Map[String, Int] = {
    entities.groupBy(_.entityType).map { case (entityType, entitiesList) =>
      entityType -> entitiesList.length
    }
  }
}
